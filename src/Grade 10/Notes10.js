...
import Sidebar from '../Sidebar10'; // Import the Sidebar component
...
const Grade10Notes = () => {
  ...
  /*useEffect(() => {
    const fetchNotes = async () => {
      try {
        // Query to fetch notes only for Grade 10
        const q = query(collection(db, 'notes10'), where('grade', '==', 10));
  ...
  const groupedNotesByInstitution = (notesData) => {
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(collection(db, 'notes10'), (querySnapshot) => {
          const notesData = querySnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
          }));
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <header className="mb-12 mx-8">
          <h2 className="text-3xl font-semibold text-black"> Notes - Grade 10</h2>

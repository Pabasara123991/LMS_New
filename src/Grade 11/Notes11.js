...
import Sidebar from '../Sidebar11'; // Import the Sidebar component
...
const Grade11Notes = () => {
 ...
 /* useEffect(() => {
  ...
  const groupedNotesByInstitution = (notesData) => {
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(collection(db, 'notes11'), (querySnapshot) => {
          const notesData = querySnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
          }));
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <header className="mb-12 mx-8">
          <h2 className="text-3xl font-semibold text-black">Notes - Grade 11</h2>

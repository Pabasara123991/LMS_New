  ...
  /*useEffect(() => {
    const fetchNotes = async () => {
      try {
        ...
        const q = query(collection(db, 'notes6'), where('grade', '==', 6));
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <header className="mb-12 mx-8">
          <h2 className="text-3xl font-bold text-black">Notes - Grade 6</h2>
        ...
        {Object.entries(notesByInstitution).map(([institution, notes]) => (
          <section key={institution} className="mb-12 mx-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {notes.map((note) => (
                <div key={note.id} className="bg-white rounded-lg shadow-md  transition-all duration-300 hover:shadow-lg">
                  <div className="p-6">

...
import Sidebar from '../Sidebar7'; // Import the Sidebar component
...
const Grade7Notes = () => {
  ...
  /*useEffect(() => {
    const fetchNotes = async () => {
      try {
        // Query to fetch notes only for Grade 7
        const q = query(collection(db, 'notes7'), where('grade', '==', 7));
  ...
  const groupedNotesByInstitution = (notesData) => {
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(collection(db, 'notes7'), (querySnapshot) => {
          const notesData = querySnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
          }));
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <header className="mb-12 mx-8">
          <h2 className="text-3xl font-bold text-black"> Notes - Grade 7</h2>

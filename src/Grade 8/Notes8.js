...
import Sidebar8 from '../Sidebar8'; // Import the Sidebar component
...
const Grade8Notes = () => {
 ...
 /* useEffect(() => {
    const fetchNotes = async () => {
      try {
        // Query to fetch notes only for Grade 8
        const q = query(collection(db, 'notes8'), where('grade', '==', 8));
  ...
  const groupedNotesByInstitution = (notesData) => {
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(collection(db, 'notes8'), (querySnapshot) => {
          const notesData = querySnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
          }));
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <Sidebar8 activeItem="Notes" />

      <main className="flex-1 overflow-y-auto p-6">
        <header className="mb-12 mx-8">
          <h2 className="text-3xl font-bold text-black">Notes- Grade 8</h2>
